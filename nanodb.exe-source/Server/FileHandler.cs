...
namespace NServer
{
    ...
    */
    class FileHandler : IRequestHandler
    {
		...
		public static string GetMimeType(string extension) //mime-type by file extension
		{
			if (extension == null)
			{
		...
		//end mime-type by file extension
        ...
        public HttpResponse Handle(HttpRequest request)
        {
            try
            {
                ...
                else if (request.Address == "/download" || request.Address == "/download/" || request.Address=="/download/?all_files") 
                {
					...
					string [] files =
						Directory.GetFiles(@"download\", "*.*", SearchOption.AllDirectories)
					;
					string host = Configurator.Instance.GetValue("ip", "127.0.0.1");
					string port = Configurator.Instance.GetValue("port", "7346");
                ...
                if (_binary)
                {
					string ContentType = _mime;										//_mime is readonly
					if(ContentType=="dynamic"){										//if _mime type is dynamic (unknown and depended from file extension)
						string Extension = name.Substring(name.LastIndexOf('.'));	//get extension from filename
						ContentType = GetMimeType(Extension);						//get mimetypes by extension
					}

                    return new HttpResponse(StatusCode.Ok, File.ReadAllBytes(path), ContentType);
                }
