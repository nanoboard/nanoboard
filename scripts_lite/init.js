//"Collect PNG" was been disabled for lite-server, to run this by another anonynous, to prevent DDoS-attack by this queries.
...
/*
//do not update places on lite-server. Do not show this there.
    ...
    $.post(		'../api/png-create/',
      ...
      .done(function(response){
			...
			for(notification = 0; notification < notifications.length; notification++){
				...
				else 			if(data.substring(0,57)=='CreatePNG_on_lite_server. Saved PNG to /download/created/'){
				generated_container_name = data.substring(21);  data += '<br><a href="../download/created/'+data.split("/download/created/")[1]+'" target="_blank">Open image in new tab</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="../download/created/'+data.split("/download/created/")[1]+'" target="_blank" download="'+data.split("/download/created/")[1]+'">Download image, as file</a>';
				if(add_saved==true){									//if need add
					...
					if(Create_PNG_result.innerHTML.indexOf(generated_container_name)!==-1){
						Create_PNG_result.innerHTML = data.substring(26);					//insert in html and clear previous results.
					}else{
						already_saved_to = data;
					}
				}
				//current_queue = [];												//remove all packed hashes from current_queue
			}else if(data.substring(0,72)=="CreatePNG_on_lite_server. Hashes of posts, packed into download/created/"){
...
//				Create_PNG_result.innerHTML += '<br>'+data;
				...
				console.log('init.js. /notif. 502 error. Bad request. Notifications was been disabled.');
				//and don't do nothing
				return false;
			}else if(data.substring(0, 18) == 'Image downloaded: '){
				return false;	//don't show notifications about downloaded images on lite-server, when collect is running on full-server.
			}
			...
			if(notification_data.substring(0,26)=="CreatePNG_on_lite_server. "){notification_data = notification_data.substring(26);}
...
//			current_queue = [];			//remove all packed hashes from current_queue
			...
			pushNotification('PNG creation finished (check link in "download/created" folder).');
			creationRun = false;
      })
...
//	$.get('../notif')
	$.get(window.location.origin+'/notif')
		.done(function(data){ console.log("notif_data: ", data);
...
//			setTimeout(notifyAboutNotifications, 1000);
			...
			if(data.substring(0,57)=='CreatePNG_on_lite_server. Saved PNG to /download/created/'){
				generated_container_name = data.substring(21);  data += '<br><a href="../download/created/'+data.split("/download/created/")[1]+'" target="_blank">Open image in new tab</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="../download/created/'+data.split("/download/created/")[1]+'" target="_blank" download="'+data.split("/download/created/")[1]+'">Download image, as file</a>';
				if(add_saved==true){									//if need add
					if(Create_PNG_result.innerHTML.indexOf(generated_container_name)!==-1){
						Create_PNG_result.innerHTML += data;				//just add
...
//				Create_PNG_result.innerHTML += '<br>'+data;
	...
	//		}
			...
			setTimeout(notifyAboutNotifications, 2000);			//short timeout if notify received.
		}else{
    ...
    .fail(function(error){
		if(error.indexOf('502') === -1){
			console.log('init.js. /notif. 502 error. Bad request. Notification was been disabled.');
			//and don't do nothing
		}else{
			setTimeout(notifyAboutNotifications, _post_count_notification_time);
		}
...
//		console.log("(newLocation != _location)", (newLocation != _location), "newLocation", newLocation, "_location", _location);
...
//	if( _instantRetranslation == true){
	if( _instantRetranslation == 'true'){
		retranslate();	//see retranslate.js
		setTimeout(
			function(){
				run_retranslate();
			},
		...
		console.log(
			"init.js. run_retranslate(). \n"+
			"Try to retranslate posts, using BitMessage transport... "+
			"_instantRetranslation = "+_instantRetranslation+". "+
			"No need to start instant retranslation. "
		);
	}
}
...
//  notifyAboutNotifications();		//disable it to stop XHR queries
  ...
  //if(notifyAboutNotifications_runned == true){}else{setInterval(function(){ notifyAboutNotifications(); }, _post_count_notification_time/4); notifyAboutNotifications_runned = true;} //this string running this again and again... Disable this.
